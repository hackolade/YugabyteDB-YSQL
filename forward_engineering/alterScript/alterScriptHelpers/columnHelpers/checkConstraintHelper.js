...
const getAddColumnCheckConstraintScriptDtos = (constraintHistory, fullTableName) => {
	return constraintHistory
		...
		.map(historyEntry => {
			const { name, expression } = historyEntry.new;
			const constraintName = getConstraintName(name, historyEntry.columnName, fullTableName);

			const script = addCheckConstraint(fullTableName, constraintName, expression);
	...
	return constraintHistory
		.filter(historyEntry => {
			if (historyEntry.old?.expression && historyEntry.new?.expression) {
				...
				return oldExpression !== newExpression || oldName !== newName;
			}
			return false;
		})
		.flatMap(historyEntry => {
			...
			const { name: newConstrainName, expression: newConstraintExpression } = historyEntry.new;
